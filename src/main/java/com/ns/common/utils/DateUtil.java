...
//                return 0;
     ...
     * @return
     */
    public static String addDate(String day, int value, int type) {
        SimpleDateFormat format = new SimpleDateFormat(DateUtil.DEFAULT_DATE_TIME_RFGFX);
        Date date = null;
        try {
        ...
        cal.setTime(date);
        if (type == 0) {
            cal.add(Calendar.DATE, value);// num为增加的天数，可以改变的
        }
        if (type == 1) {
