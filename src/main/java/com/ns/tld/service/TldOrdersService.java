...
import com.ns.customer.task.TwitterRebateTask;
...
public class TldOrdersService {
	...
	public static void main(String[] args) {
		...
		int b = 5;
		System.out.println(a.multiply(new BigDecimal(b)).divide(new BigDecimal(100))
	...
	private int[] compute(boolean isTitter, PntProduct pntProduct, PntSku sku) {
		...
		if (isTitter) {
			...
			if (sku != null) {
				...
				integralSelf += disAmount.multiply(new BigDecimal(sku.getIntegralSelf())).divide(new BigDecimal(100))
						.setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
				integralSup += disAmount.multiply(new BigDecimal(sku.getIntegralSup())).divide(new BigDecimal(100))
						.setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
			} else {
				if (StrKit.notBlank(re_consume_rate)) {
					disAmount = pntProduct.getSalPrice().multiply(new BigDecimal(re_consume_rate))
							.divide(new BigDecimal(10));
				} else {
					disAmount = pntProduct.getSalPrice();
				}
				integralSelf += disAmount.multiply(new BigDecimal(pntProduct.getIntegralSelf()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
				integralSup += disAmount.multiply(new BigDecimal(pntProduct.getIntegralSup()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
			}
		} else {
			if (sku != null) {
				integralSelf += sku.getSalPrice().multiply(new BigDecimal(sku.getIntegralSelf()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
				integralSup += sku.getSalPrice().multiply(new BigDecimal(sku.getIntegralSup()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
			} else {
				integralSelf += pntProduct.getSalPrice().multiply(new BigDecimal(pntProduct.getIntegralSelf()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
				integralSup += pntProduct.getSalPrice().multiply(new BigDecimal(pntProduct.getIntegralSup()))
						.divide(new BigDecimal(100)).setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
			}
		}
	...
	public Map<String, String> newOrder(String sk, JSONObject jsonObject) {
		...
		if (jsonObject.containsKey("POINT")) {
			...
			// 扣积分
			Db.update("update bas_customer_ext set POINTS_ENABLED=POINTS_ENABLED-? where con_id = ?", point,
					customer.getID());
			// 加积分流水
		...
		orders = setOrders(orders, orderId, orderNo, customer, integralSelf, integralSup, pntAmountSum, couponAmount,
				pointAmount);
		orders.save();
	...
	private TldOrderItems setItems(BasCustomer customer, String orderId, String orderNo, PntSku sku,
			PntProduct pntProduct, int quantity, BigDecimal amt) {
	...
	private TldOrders setOrders(TldOrders orders, String orderId, String orderNo, BasCustomer customer,
			int integralSelf, int integralSup, BigDecimal pntAmountSum, BigDecimal couponAmount,
		...
		BigDecimal orderTotal = disAmount.subtract(couponAmount).subtract(pointAmount).subtract(randomAmount)
				.add(orders.getFREIGHT());
		// 防止出现负数
	...
	public void orderPay(String orderId, int payType) {
		...
		if (orders.getSTATUS() == 1) {
			...
			/*
			 * //重消加自己营业总额,重消总额 if (orders.getIsReorder() == 1) {
			 * selfExt.setREVENUES(selfExt.getREVENUES().add(orders.
			 * getOrderTotal()));
			 * selfExt.setReConsumptions(selfExt.getReConsumptions().add(orders.
			 * getOrderTotal())); } //首单+上级已购买 if (orders.getIsReorder() == 0) {
			 * up1Ext.setPuredCustQty(up1Ext.getPuredCustQty() + 1); } //自己消费总额
			 * selfExt.setCONSUMPTIONS(selfExt.getCONSUMPTIONS().add(orders.
			 * getOrderTotal())); //自己订单总数量
			 ...
			 * ())); //上级+推广订单总数 up1Ext.setOrdersProm(up1Ext.getOrdersProm() +
			 * 1);
			 */
			...
			// 7:加积分流水
			if (selfIntegral > 0) {
				insertPointTrans(selfExt.getConId(), selfExt.getConNo(), selfExt.getConName(), selfExt.getConId(),
						selfExt.getConNo(), selfExt.getConName(), orderId, orders.getOrderNo(), selfIntegral, 2,
						"下单成功返积分");
			}
			if (up1Integral > 0) {
				insertPointTrans(up1Ext.getConId(), up1Ext.getConNo(), up1Ext.getConName(), selfExt.getConId(),
						selfExt.getConNo(), selfExt.getConName(), orderId, orders.getOrderNo(), up1Integral, 2,
						"下单成功返积分");
			}
			...
			if (orders.getIsReorder() == 0) {
				...
				if (StrKit.notBlank(subscribe_points_up1)) {
					if (up1Ext != null) {
						...
						if (subscribePointsUp1 > 0) {
							...
							insertPointTrans(up1Ext.getConId(), up1Ext.getConNo(), up1Ext.getConName(),
									selfExt.getConId(), selfExt.getConNo(), selfExt.getConName(), orderId,
									orders.getOrderNo(), Integer.valueOf(subscribe_points_up1), 1, "新增会员积分");
						}
					}
				}
				// 自己加关注推荐积分
				...
				if (StrKit.notBlank(subscribe_points_self)) {
					...
					if (Integer.valueOf(subscribePointsSelf) > 0) {
						...
						insertPointTrans(selfExt.getConId(), selfExt.getConNo(), selfExt.getConName(),
								selfExt.getConId(), selfExt.getConNo(), selfExt.getConName(), orderId,
								orders.getOrderNo(), Integer.valueOf(subscribe_points_self), 1, "新增会员积分");
					}
				}
			...
			// 发送模板消息
			noticeService.getOrderPaySuccessNotice(customer.getID(), customer.getOPENID(), orders.getOrderNo(),
					orders.getPayDt(), String.valueOf(orders.getOrderTotal()), orders.getPaymentType());
			// 自己积分变动消息模板
			if (selfIntegral > 0 || subscribePointsSelf > 0) {
				noticeService.getPointsNotice(customer.getID(), customer.getOPENID(), customer.getConName(),
						orders.getPayDt(), String.valueOf(selfIntegral + subscribePointsSelf),
						String.valueOf(selfExt.getPointsTotal()));
			}
			// 上级积分变动消息模板
			if (up1Integral > 0 || subscribePointsUp1 > 0) {
				String opneId = Db.queryStr("select OPENID from bas_customer where ID = ?", customer.getRpId());
				noticeService.getRpPointsNotice(customer.getRpId(), opneId, customer.getRpName(), customer.getConNo(),
						orders.getPayDt(), String.valueOf(up1Integral + subscribePointsUp1),
						String.valueOf(up1Ext.getPointsTotal()));
			}

			// 新增.返利模块.
			int status = flowService.STATUS_WAITING;
			Task task = new TwitterRebateTask(selfExt, orders, status);
	...
	private void split(TldOrders orders) {
		List<Record> orderItems = Db.find(
				"select QUANTITY,PNT_ID,PNT_NAME,SKU_ID,SKU_NAME,SALE_PRICE from tld_order_items where order_id = ?",
				orders.getID());
		int splitNum = 1;// 分单数量
		for (Record items : orderItems) {
			...
			if (quantity < splitNum) {
				inertOrderSplit(orders, items.getStr("PNT_ID"), items.getStr("PNT_NAME"), items.getStr("SKU_ID"),
						items.getStr("SKU_NAME"), splitNum, items.getBigDecimal("SALE_PRICE"));
			} else {
				while (quantity > tempNum) {
					if (quantity - tempNum > splitNum) {
						inertOrderSplit(orders, items.getStr("PNT_ID"), items.getStr("PNT_NAME"),
								items.getStr("SKU_ID"), items.getStr("SKU_NAME"), splitNum,
								items.getBigDecimal("SALE_PRICE"));
					} else {
						inertOrderSplit(orders, items.getStr("PNT_ID"), items.getStr("PNT_NAME"),
								items.getStr("SKU_ID"), items.getStr("SKU_NAME"), splitNum,
								items.getBigDecimal("SALE_PRICE"));
	 ...
	 * @param remark
	 */
	private void insertPointTrans(String conId, String conNo, String conName, String fromConId, String fromConNo,
			String fromConName, String orderId, String orderNo, Integer pointsQty, Integer pointsType, String remark) {
	...
	public Object getOrderList(int pageNumber, int pageSize, String conId, Integer status) {
		StringBuffer sqlExceptSelect = new StringBuffer(
				" from tld_orders where ENABLED = 1 and status !=12 and con_id = '" + conId + "'");
		if (status != null) {
			// 待收货应该包含 已打印和配送中
			if (status == 6) {
		...
		Page<Record> tldOrdersPage = Db.paginate(pageNumber, pageSize, "select " + COLUMN + "",
				sqlExceptSelect.toString());
		for (Record order : tldOrdersPage.getList()) {
			String orderId = order.get("ID");
			List<Record> items = Db.find(
					"select PNT_ID,SKU_ID,PNT_NAME,SKU_NAME,QUANTITY,SALE_PRICE,COMMENT_TAG from tld_order_items where ORDER_ID=?",
					orderId);
			for (Record item : items) {
				final String pntId = item.getStr("PNT_ID");
				final String skuId = item.getStr("SKU_ID");
				if (StrKit.notBlank(skuId)) {
					// List<Record> items = Db.find("select t1.*,t2.THUMB_URL
					// from tld_order_items t1 left join pnt_sku t2 on t1.sku_id
					// = t2.id where t1.ENABLED = 1 and t1.order_id = ?",
					// orderId);
	...
	public void updateOrderStatus13(String conId) {
		List<Record> list = Db
				.find("select " + COLUMN + " from tld_orders where ENABLED = 1 and status=7 and con_id = ?", conId);
		for (Record order : list) {
			String orderId = order.get("ID");
			Integer count = Db.queryInt("select count(1) from tld_order_items where COMMENT_TAG = 0 and order_id = ? ",
					orderId);
	...
	public Record getOrderStatusNum(String conId) {
		Record record = Db.findFirst(
				"SELECT " + " sum(CASE STATUS WHEN 1 THEN 1 ELSE 0 END ) 'DFK',"
						+ " sum(CASE WHEN STATUS IN (5, 6) THEN 1 ELSE 0 END ) 'DSH', "
						+ " sum(CASE STATUS WHEN 7 THEN 1 ELSE 0 END ) 'DPJ' " + "FROM tld_orders where con_id = ?",
				conId);
		return record;
	}
	...
	public Record getOrderItems(String orderId) {
		...
		record.set("ITEMS",
				Db.find("select t1.*,t2.THUMB_URL from tld_order_items t1 left join pnt_sku t2 on t1.sku_id = t2.id where t1.ENABLED = 1 and t1.order_id = ?",
						orderId));
		return record;
	}
	...
	public List<Record> getOrderSplit(String orderId) {
		return Db.find(
				"select EXP_COMPANY_ID,EXP_COMPANY_NAME,WAYBILL from tld_order_split  where ENABLED = 1 and order_id = ?",
				orderId);
	}
	...
	public boolean deleteOrder(String orderId) {
		...
		if (orders.getSTATUS() == 11) {
			Db.update("update tld_orders set STATUS = 12,VERSION = VERSION +1,UPDATE_DT = ?,REMARK = ? where id = ?",
					DateUtil.getNow(), "", orderId);
		} else {
			...
			// 查询该订单是否使用积分抵扣
			Record pointGrans = Db.findFirst(
					"select ID,POINTS_QTY from bas_cust_point_trans where POINTS_TYPE = 3 and FROM_ORDER_ID = ?",
					orderId);
			if (pointGrans != null) {
			...
			String remark = "";
			Db.update("update tld_orders set STATUS = 12,VERSION = VERSION +1,UPDATE_DT = ?,REMARK = ? where id = ?",
					DateUtil.getNow(), remark, orderId);
			// 还原库存
			List<Record> itemsList = Db.find("select SKU_ID,PNT_ID,QUANTITY from tld_order_items where order_id = ?",
					orderId);
			Jedis cache = Redis.use().getJedis();
			try {
	...
	public boolean refund(String orderId) {
		...
		// 查询该订单是否使用积分抵扣
		Record pointGrans = Db.findFirst(
				"select ID,POINTS_QTY from bas_cust_point_trans where POINTS_TYPE = 3 and FROM_ORDER_ID = ?", orderId);
		if (pointGrans != null) {
			int point = pointGrans.getInt("POINTS_QTY");
			Db.update("update bas_customer_ext set POINTS_ENABLED=POINTS_ENABLED+? where con_id = ?", point, conId);
			Db.delete("delete FROM  bas_cust_point_trans where id = ?", pointGrans.getStr("ID"));
		...
		for (BasCustPointTrans pointTrans : list) {
			if (pointTrans.getPointsType() == 1 || pointTrans.getPointsType() == 2) {
				...
				insertPointTrans(pointTrans.getConId(), pointTrans.getConNo(), pointTrans.getConName(),
						pointTrans.getFromConId(), pointTrans.getFromConNo(), pointTrans.getFromConName(), orderId,
						orders.getOrderNo(), -pointTrans.getPointsQty(), pointTrans.getPointsType(), "订单退款,积分红冲");
			}
		}
		...
		if (selfIntegral > 0) {
			// -总积分和未确认积分
			Db.update(
					"update bas_customer_ext set POINTS_TOTAL=POINTS_TOTAL-?,POINTS_UNCFMD=POINTS_UNCFMD-? where con_id = ?",
					selfIntegral, selfIntegral, orders.getConId());
		}
		if (up1Integral > 0) {
			// -总积分和未确认积分
			Db.update(
					"update bas_customer_ext set POINTS_TOTAL=POINTS_TOTAL-?,POINTS_UNCFMD=POINTS_UNCFMD-? where con_id = ?",
					up1Integral, up1Integral, orders.getRpId());
		}

		String remark = "未发货已退款";
		Db.update("update tld_orders set STATUS = 10,VERSION = VERSION +1,UPDATE_DT = ?,REMARK = ? where id = ?",
				DateUtil.getNow(), remark, orderId);
		...
		// 新增.返利模块.撤单.
		int status = flowService.STATUS_DISABLE;
		Task task = new TwitterRebateTask(null, orders, status);
		TaskQueuePlugin.use("default").putTask(task);
		return true;
	}
	...
	public boolean confirmOrder(String orderId) {
		...
		if (orders.getSTATUS() == 5 || orders.getSTATUS() == 6) {
			...
			orders.setSTATUS(7);
			orders.update();
			Db.update("update tld_order_split set status = 7,VERSION=VERSION+1,UPDATE_DT = ? where order_id = ?",
					dateTime, orderId);
			...
			noticeService.sendOrderReceivedNotice(orders.getConId(), opneId, orders.getOrderNo(), orders.getConfirmDt(),
					String.valueOf(orders.getOrderTotal()));

			// 新增.返利模块.
			// 新增.返利模块.完成订单返利操作.
			int status = flowService.STATUS_FINISH;
			Task task = new TwitterRebateTask(selfExt, orders, status);
			TaskQueuePlugin.use("default").putTask(task);
		} else {
	...
	public void rebate(BasCustomerExt selfExt, TldOrders orders, int status) {
		TldTwitter t = twitterService.findDirectTwitter(orders.getConNo(), orders.getRpNo());
		if (flowService.STATUS_WAITING == status) {
